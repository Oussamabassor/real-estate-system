...
const isApiConnected = () => {
  return localStorage.getItem('apiConnectionStatus') === 'connected';
};

// Wrap axios requests to handle offline state
const apiRequest = async (method, url, data = null, options = {}) => {
  // If we know the API is offline, fail fast
  if (!isApiConnected() && !options.bypassOfflineCheck) {
    throw new Error('API is offline. Using cached data.');
  }
...
export const authApi = {
  login: (credentials) => apiRequest('post', '/api/login_new.php', credentials),
